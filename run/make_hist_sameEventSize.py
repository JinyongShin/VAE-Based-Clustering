...
for i in range(0,len(np.unique(df['Label'].to_numpy()))):
	if i <= 9:
		n_la.append(str(i))
		n_re.append(df['RE'][df['Label']==i][:70].to_numpy())
		n_kld.append(df['KLD'][df['Label']==i][:70].to_numpy())
...
#plt.show()
plt.savefig(args.indir + '/stacked_re_SameSize.png')
plt.close()

an_kld_c = np.concatenate(an_kld)
#plt.hist(n_kld,label=n_la,color=c_lst,range=(0,100),bins=50,stacked=True)
plt.hist(n_kld,label=n_la,color=c_lst,range=(0,100),bins=10,stacked=True)
plt.hist(an_kld_c,label='Anomaly',histtype='step',color='r',linewidth=2,range=(0,100),bins=10)
#plt.yscale('log')
plt.legend()
plt.xlabel('KL-Divergence')
plt.ylabel('Number of events / bin')
#plt.show()
plt.savefig(args.indir + '/stacked_kld_SameSize.png')
plt.close()
...
plt.hist(n_kld,label='normal',color='royalblue',range=(0,100),bins=50)
plt.hist(an_kld,label='Anormaly',histtype='step',color='r',linewidth=2,range=(0,100),bins=50)
plt.legend()
plt.yscale('log')
