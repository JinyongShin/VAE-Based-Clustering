...
parser.add_argument('--model', action='store',choices=('model_v1','model_v2','mlp','mlp_vade'),default='default',help='choice of model')
parser.add_argument('--ldim',action='store',type=int,default=2,help='latent dim')
parser.add_argument('--nClusters',action='store',type=int,default=10)
...
z = model.reparameterize(enc1,enc2)
...
#	print('Class ',i,'y :',tsneArr[test_la==i,1])
	plt.scatter(z_test[test_la==i,0],z_test[test_la==i,1],label=i,color=c_lst[i])
plt.legend(loc='best')
plt.title('2D Latent')
#plt.show()
plt.savefig(args.outdir + '/latent_direct_scatter.png')
